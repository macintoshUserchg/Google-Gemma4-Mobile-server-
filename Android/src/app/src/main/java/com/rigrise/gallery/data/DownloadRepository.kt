...
import com.rigrise.gallery.worker.DownloadWorker
...
class DefaultDownloadRepository(
  ...
  override fun downloadModel(
    ...
    // Calculate total bytes if not already set, otherwise use pre-calculated model.totalBytes
    val totalBytes = if (model.totalBytes > 0) model.totalBytes else (model.sizeInBytes + model.extraDataFiles.sumOf { it.sizeInBytes })
    
    // Check available storage space before starting download (with 50MB buffer)
    val externalFilesDir = context.getExternalFilesDir(null)
    val usableSpace = externalFilesDir?.usableSpace ?: 0L
    val requiredSpace = totalBytes + (50 * 1024 * 1024) // 50MB buffer
    
    if (usableSpace < requiredSpace) {
      Log.e(TAG, "Insufficient storage space for ${model.name}. Required: $requiredSpace, Available: $usableSpace")
      onStatusUpdated(
        model,
        ModelDownloadStatus(
          status = ModelDownloadStatusType.FAILED,
          errorMessage = "Insufficient storage space. Required: ${requiredSpace / 1024 / 1024}MB, Available: ${usableSpace / 1024 / 1024}MB"
        )
      )
      return
    }
  ...
  private fun sendNotification(title: String, text: String, taskId: String, modelName: String) {
    ...
    val channelName = "Rigrise AI Edge download notification"
    ...
    // Download from global model manager. Open the global model manager screen.
      ...
      intent =
        Intent(Intent.ACTION_VIEW, "com.rigrise.gallery://global_model_manager".toUri())
      ...
      intent =
        Intent(
            Intent.ACTION_VIEW,
            "com.rigrise.gallery://model/$taskId/${modelName}".toUri(),
          )
          .apply { flags = Intent.FLAG_ACTIVITY_NEW_TASK }
    }
