...
package com.rigrise.gallery.ui.modelmanager
...
import com.rigrise.gallery.BuildConfig
import com.rigrise.gallery.R
import com.rigrise.gallery.common.ProjectConfig
import com.rigrise.gallery.common.getJsonResponse
...
import com.rigrise.gallery.data.CategoryInfo
import com.rigrise.gallery.data.Config
...
import com.rigrise.gallery.data.EMPTY_MODEL
import com.rigrise.gallery.data.IMPORTS_DIR
import com.rigrise.gallery.data.Model
import com.rigrise.gallery.data.ModelAllowlist
import com.rigrise.gallery.data.ModelDownloadStatus
import com.rigrise.gallery.data.ModelDownloadStatusType
import com.rigrise.gallery.data.NumberSliderConfig
import com.rigrise.gallery.data.RuntimeType
import com.rigrise.gallery.data.SOC
import com.rigrise.gallery.data.TMP_FILE_EXT
import com.rigrise.gallery.data.Task
import com.rigrise.gallery.data.ValueType
import com.rigrise.gallery.data.createLlmChatConfigs
import com.rigrise.gallery.proto.AccessTokenData
...
constructor(
  ...
  private val modelCacheManager: com.rigrise.gallery.data.ModelCacheManager,
  ...
  fun getAllModels(): List<Model> {
    ...
    // Use both tasks from UI state and directly from customTasks to be safe.
    val tasksToCheck = mutableSetOf<Task>()
    tasksToCheck.addAll(uiState.value.tasks)
    tasksToCheck.addAll(getActiveCustomTasks().map { it.task })
  ...
  /** Sync experimental tasks with downloaded models. */
  fun updateExperimentalTasks(downloadStatus: Map<String, ModelDownloadStatus>? = null) {
    val modelServerTask = getCustomTaskByTaskId("model_server")?.task ?: return
    val status = downloadStatus ?: uiState.value.modelDownloadStatus

    val allModels = getAllModels()
    val downloadedModels = allModels.filter { model ->
      val modelStatus = status[model.name]
      val isDownloaded = modelStatus?.status == ModelDownloadStatusType.SUCCEEDED
      val isLlm = model.isLlm

      Log.d(TAG, "Checking model for server: ${model.name}, downloaded: $isDownloaded, isLlm: $isLlm")
      isDownloaded && isLlm
    }

    Log.d(TAG, "Updating model_server task with ${downloadedModels.size} models. Total models checked: ${allModels.size}")

    modelServerTask.models.clear()
    modelServerTask.models.addAll(downloadedModels)
    modelServerTask.updateTrigger.value = System.currentTimeMillis()
  }
  ...
  fun downloadModel(task: Task?, model: Model) {
    // Prefer ProjectConfig token first (hardcoded, more reliable)
    if (ProjectConfig.HF_ACCESS_TOKEN.isNotEmpty()) {
      model.accessToken = ProjectConfig.HF_ACCESS_TOKEN
      Log.d(TAG, "Using access token from ProjectConfig for '${model.name}'")
    } else {
      // Fallback to DataStore token
      ...
      if (tokenStatusAndData.status == TokenStatus.NOT_EXPIRED && tokenStatusAndData.data != null) {
        ...
        Log.d(TAG, "Using access token from DataStore for '${model.name}'")
      }
    }
    ...
    viewModelScope.launch(Dispatchers.Default) {
      ...
      val onDoneFn: (error: String) -> Unit = { error ->
        ...
        if (model.instance != null) {
          ...
          // Register the initialized model to the ModelCacheManager for lifecycle/memory cleanup
          modelCacheManager.registerModel(model) {
            cleanupModel(context = context, task = task, model = model)
          }
  ...
  fun cleanupModel(
    ...
    if (model.instance != null) {
      ...
      val onDoneFn: () -> Unit = {
        ...
        // Unregister the model from cache manager as it's been cleaned up
        modelCacheManager.unregisterModel(model)
  ...
  fun setDownloadStatus(curModel: Model, status: ModelDownloadStatus) {
    ...
    if (status.status == ModelDownloadStatusType.SUCCEEDED) {
      updateExperimentalTasks(downloadStatus = curModelDownloadStatus)
    }
  ...
  fun getTokenStatusAndData(): TokenStatusAndData {
    ...
    var tokenData = dataStoreRepository.readAccessTokenData()
    ...
    if (tokenData != null && tokenData.accessToken.isNotEmpty()) {
      ...
      Log.d(TAG, "Token doesn't exist in DataStore.")
    }

    // If no valid token in DataStore, check ProjectConfig for hardcoded HF token
    if (tokenStatus != TokenStatus.NOT_EXPIRED) {
      if (ProjectConfig.HF_ACCESS_TOKEN.isNotEmpty()) {
        Log.d(TAG, "Using hardcoded token from ProjectConfig as fallback.")
        tokenStatus = TokenStatus.NOT_EXPIRED
        curAccessToken = ProjectConfig.HF_ACCESS_TOKEN
        // Create a tokenData object for the ProjectConfig token
        tokenData =
          AccessTokenData.newBuilder()
            .setAccessToken(ProjectConfig.HF_ACCESS_TOKEN)
            .setRefreshToken("")
            .setExpiresAtMs(Long.MAX_VALUE) // Never expires
            .build()
      }
    }
  ...
  private fun createUiState(): ModelManagerUiState {
    ...
    updateExperimentalTasks(downloadStatus = modelDownloadStatus)
